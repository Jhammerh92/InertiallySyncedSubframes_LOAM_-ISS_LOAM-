...
void inline fromROSMsg(const sensor_msgs::msg::PointCloud2 &cloud, pcl::PointCloud<PointTypeNoNormals> &pcl_cloud)
{   
...
void inline toROSMsg( const pcl::PointCloud<PointType> &pcl_cloud, sensor_msgs::msg::PointCloud2 &msg)
{   
...
builtin_interfaces::msg::Time inline toHeaderStamp(double time)
{
    ...
    int secs = (int)std::floor(time);
    int nanosecs = (int)((time - secs)*1e9);
    stamp.set__sec(secs);
    stamp.set__nanosec(nanosecs);
